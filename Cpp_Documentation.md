...
#### Clean Iteration (Using Auto)
Because vectors track their own size, we no longer need to pass length variables around like we did with C arrays. C++ interoduced the Range-Based for loop, which pairs perfectly with vectors and the `auto` keyword.
...
Vectors own Heap memory. Therefore, passing them by value creates a Deep Copy (a massive, slow duplication of the entire array).

Rule: Always pass vectors by reference (`&`), and use `const` if the function shouldn't modify the data.

```cpp
// BAD: Triggers a massive heap allocation and copy of all 10,000 floats!
void analyzeData(std::vector<float> data) { ... }

// GOOD: Passes an 8-byte pointer under the hood. Zero copy cost.
void analyzeData(const std::vector<float>& data) { ... }
```

#### Combining Vectors and Smart Pointers
In earlier lessons, you built an `ISensor` HAL and created an array of pointers to achieve Polymorphism. Raw arrays are dangerous. In Modern C++, you use a `std::vector` of `std::unique_ptr` to manage polymorphic hardware!
This pattern automatically destroys the sensor objects when the vector is destroyed.
...
#### Removing Elements
You can remove elements from a vector using `.erase()`. However, because vectors must remain contiguous, removing an element from the middle of the vector forces the CPU to shift every subsequent element to the left to close the gap.
...
```text
========== TOPIC: Dynamic Arrays in C++ ==========
Size of the numbers vector: 4
Capacity of the numbers vector: 4
All Elements in the Vector: 10 20 30 40 
Size of the numbers vector: 3
Capacity of the numbers vector: 4
All Elements in the Vector: 10 30 40 
Size of the numbers vector: 0
Capacity of the numbers vector: 4
...
- Discusses about how vector's work internally and why there is lots of copy in vectors and how we can optimize this.
- Implements examples of catching copying operation when using vectors
- Discussed about `.reserve()` and `emplace_back()` to optimize the use of vectors in C++.

- Summary: The Cherno demonstrates how to optimize C++ vector usage by reducing unnecessary object copies through understanding memory allocation. Techniques like reserving capacity and using emplace back are explored to improve performance.
...
```text
Output Log:
[100%] Built target Cpp032_dynamic_arrays
========== TOPIC: Dynamic Arrays in C++ ==========
Copied Occured!
Copied Occured!
Copied Occured!
...
Copied Occured!
Copied Occured!
Optimization implemented below, therefore no copies will be printed below
